...
function getTravelRecommendation(e) {
  // todo
  //先找到 station Element的父層 元素
  console.log(e.target.parentElement.parentElement);
  const stationElement = e.target.parentElement.parentElement;
  //找stationName, stationLocation
  const stationNameEl = stationElement.parentElement.querySelector(".station-name");
  const stationLocationEl = stationElement.parentElement.querySelector(".station-location");
  console.log(stationNameEl.innerText.split(":")[1]); //通北街65巷口
  console.log(stationLocationEl.innerText.split(":"[1])); //通北街/通北街65巷口(東北側)
  console.log("get travel recommendation")
  const travelRecommendEl = document.querySelector("#travel-recommendation-info");

  const stationName = stationNameEl.innerHTML.split(":")[1];
  const stationLocation = stationLocationEl.innerHTML.split(":")[1];

  //Postman上API複製的JavaScript-Fetch格式
  const myHeaders = new Headers();
myHeaders.append("Authorization", "Bearer app-xVZMAT3ymLGD8QbZw4D4Wgfh");
myHeaders.append("Content-Type", "application/json");

const raw = JSON.stringify({
  inputs: {
    location: `${stationNameEl.innerText}`,
    station_name: `${stationLocationEl.innerText}`,
  },
...
const requestOptions = {
  ...
  headers: myHeaders,
  body: raw,
  redirect: "follow" //目前沒用途, 可刪除
};

fetch("https://agent.build-school.com/v1/workflows/run", requestOptions)
  .then((response) => response.json()) //text改為json 需要json檔
  .then((result) => (travelRecommendEl.innerText = result.data.outputs.text))
    // console.log(result.data.outputs.text)
